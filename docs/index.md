...
[StealthChanger](stealthchanger) is the key componant of a larger ecosystem that can be used on Voron and other front mount printers. StealthChanger enables the tool to be completely removed from the gantry making it proficient in tool changing when combined with the rest of the DraftShift Design ecosystem.
...
### Modular Dock
The [Modular Dock](hardware/modular_dock) gives you a place to rest the tools while they are not in use. Its modular design allows for flexible configurations to match any printer and toolhead combination.
...
### Tophat
The [Tophat](hardware/tophat) raises the top panel of the printer to allow room for the tool's umbilicals. There are both [3d printed](hardware/tophat/#printed-tophat) and [extrusion](hardware/tophat/#extrusion-tophat) options available.

<div style="clear: both; margin-bottom: 20px;"></div>
![Cable Management](/assets/cable_management.png){ width=250 align=right } 
### Cable Management
Proper [Cable Management](hardware/cable_management) plays a critical role in the performance and reliability of [StealthChanger](hardware/stealthchanger), though it is frequently underestimated. The [Cable Management](hardware/cable_management) section provides guidance on managing [umbilicals](hardware/cable_management/#umbilicals) for tools, integrating the [Fanny Pack](hardware/cable_management/#fanny-pack) to house the distribution board, and utilizing [wire ducts](hardware/cable_management/#wire-ducts) to maintain cable organization.
